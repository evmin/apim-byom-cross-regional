...
// - APIM MI -> Cognitive Services OpenAI User on SC Foundry account
// - WE project MI -> Cosmos / AI Search / Storage
// - Cosmos DB Built-in Data Contributor
// - Foundry admin-connected model

// AVM gap: the AVM rg-scope role-assignment module at pin 0.1.1 has a
...
// NATIVE FALLBACKS (with inline justifications):
...
// CRITICAL: the ONLY correct role for APIM's authentication-managed-identity
...
// WE project MI -> Cosmos / Search / Storage
...
// role assignments. Native fallback authored in `cosmos-sql-role-assignment.bicep`
...
// Foundry admin-connected model on the WE project

// AVM gap: AVM `cognitive-services/account@0.14.2` does not author
// `accounts/projects/connections` children. Native fallback. Retire when
// AVM ships first-class connections coverage.

// The connection must be authored AFTER:
// - the WE project exists,
// - the APIM service exists,
// - the APIM MI has the OpenAI User role on the SC Foundry account,
// - the cross-region inbound PE exists,
// - the `privatelink.azure-api.net` DNS zone is linked to the WE VNet.
...
// Foundry standard-agent-setup wiring
